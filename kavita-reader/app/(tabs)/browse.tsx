...
import { kavitaAPI, Genre, Tag, Series } from '../../services/kavitaAPI';
...
export default function BrowseScreen() {
  ...
  async function loadSeriesByGenre(genre: Genre, pageNum: number) {
    setSeriesLoading(true);
    try {
      const data = await kavitaAPI.getSeriesByGenre(genre.id, pageNum, 30);
      if (pageNum === 0) {
        setSeries(data);
      } else {
  ...
  async function loadSeriesByTag(tag: Tag, pageNum: number) {
    setSeriesLoading(true);
    try {
      const data = await kavitaAPI.getSeriesByTag(tag.id, pageNum, 30);
      if (pageNum === 0) {
        setSeries(data);
      } else {
        setSeries(prev => [...prev, ...data]);
  ...
  const onSelect = filterMode === 'genre'
    ? (item: Genre | Tag) => selectGenre(item as Genre)
    : (item: Genre | Tag) => selectTag(item as Tag);
